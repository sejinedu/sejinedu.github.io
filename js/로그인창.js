...
(() => {
  ...
  function 걸음보이기(새걸음) {
    ...
    if (걸음 === "로그인") {
      ...
      건너가기([["처음이면 가입", "가입"], ["비밀번호를 잊었다 — 메일 번호로 들어가기", "번호로그인"]]);
      setTimeout(() => (메일.value ? 비번 : 메일).focus(), 30);
    } else if (걸음 === "번호로그인") {
      제목.textContent = "메일 번호로 들어가기";
      설명.textContent = "가입한 메일을 넣으면 여섯 자리 번호를 보낸다.";
      보일칸("메일"); 하기.textContent = "번호 받기";
      건너가기([["비밀번호로 로그인", "로그인"]]);
      setTimeout(() => 메일.focus(), 30);
    } else if (걸음 === "번호로그인확인" || 걸음 === "가입확인") {
      제목.textContent = 걸음 === "가입확인" ? "가입 — 메일 확인" : "메일 번호로 들어가기";
