...
const 자막 = (() => {
  ...
  function 재서정한크기(쓸폭) {
    //  사용자가 고른 크기를 **천장**으로 삼는다 (배율 1.05 = 보통)
    let 배 = parseFloat(getComputedStyle(document.documentElement)
                       .getPropertyValue("--자막배율"));
    if (!(배 > 0)) 배 = 1.05;
    const 천장 = 쓸폭 * 0.030 * 배;          // 보통일 때 한 줄에 32자쯤
    if (!잰가장넓이) return 천장;             // 아직 못 쟀으면 천장으로
    //  100px 로 그렸을 때 잰가장넓이 였으니, 쓸폭에 맞추려면 이 크기다
    const 맞는크기 = 100 * (쓸폭 * 0.985) / 잰가장넓이;
    //  ★ 바닥을 둔다 — 유난히 긴 줄 하나 때문에 강의 전체 글씨가 작아지면 안 된다.
    //    그 몇 줄은 차라리 두 줄로 접는다. 접는 자리는 **말 끊기는 데**로 고른다.
    const 바닥 = 쓸폭 * 0.021;
    return Math.min(천장, Math.max(바닥, 맞는크기));
  }
