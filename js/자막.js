...
const 자막 = (() => {
  ...
  //  ★★★ 한 문장은 되도록 한 줄로 (2026-09-03 · 사용자가 정함)
  //
  //    사용자: 「야 이런게 왜 2줄로 나오냐」 「한문장은 되도록 1줄로 나와야지」
  //            「혹시 뭐 좌우 폭의 한계가 있냐?」   ← 있었다.
  //
  //    ★ 왜 두 줄이 됐나 — 세 가지가 곱해진다.
  //        ① 자막칸 좌우 한계 (스타일.css 의 max-width)
  //        ② 글자 크기 (2.9cqw, 최대 34px)
  //        ③ **사람이 고른 자막 크기 배율** — 「아주 크게」 면 1.6배다
  //      ②×③ 이 커지면 ① 안에 못 들어가서 접힌다.
  //      「이거를 면적 속도 일정의 법칙이라고 얘기를 합니다.」 26자가 그랬다.
  //
  //    ⇒ 폭을 늘리는 것만으로는 모자란다. **글씨를 조금 줄여서 한 줄에 넣는다.**
  //      ★ 사람이 고른 크기를 **웃돌지는 않는다** — 줄이기만 한다.
  //      ★ 너무 줄이면 안 보이니 78% 까지만. 그래도 안 되면 두 줄로 간다.
  const 가장작게 = 0.78;

  function 한줄에넣어보기(글) {
    자막층.style.fontSize = "";               // 사람이 고른 크기로 되돌리고 시작
    if (!글) return;
    const 들어갈수 = 한줄글자수();
    if (!들어갈수 || 글.length <= 들어갈수) return;   // 이미 한 줄이면 그만
    const 줄일만큼 = 들어갈수 / 글.length;
    if (줄일만큼 < 가장작게) return;            // 여기까지 줄여도 안 되면 두 줄로
    const 지금 = parseFloat(getComputedStyle(자막층).fontSize) || 20;
    자막층.style.fontSize = (지금 * 줄일만큼).toFixed(2) + "px";
  }
