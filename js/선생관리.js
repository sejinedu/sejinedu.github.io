...
//  ★ 학원이 없는 사람은 누구나 「학원 만들기」 로 연다 (2026-09-24 스스로 원장). 정지된 학원은 막는다.
...
const 선생관리 = (() => {
  ...
  async function 열기() {
    ...
    await 회원.학원읽기();
    const 학 = 학원();
    if (!학 || !학.학원) return 학원만들기그리기();
    if (학.상태 !== "활성") {
      머리그리기("선생 관리");
      칸.appendChild(만들기("p", "회원말 탈", 학.상태 === "정지" ? "학원이 정지됐다 — 관리자에게 물어라." : "아직 학원에 안 들어왔다."));
      return;
    }
  ...
  function 학원만들기그리기() {
    머리그리기("선생 관리", "학원 만들기");
    // (2026-09-24) 누구나 만든다 — 스스로 원장. 새 학원은 빈 것에서 시작하고 저장은 1GB. 홈피 등급은 안 오른다.
    칸.appendChild(만들기("p", "선생풀이", "아직 학원이 없다. 학원을 만들면 원장이 되고, 선생님을 초대할 수 있다.\n런처에서 학생관리 · 문제은행 · 디자인을 바로 쓴다. 저장 공간은 학원마다 1GB."));
    const 틀 = 만들기("div", "선생틀");
