...
const 재생기 = (() => {
  ...
  function 사라질시계걸기(얼마) {
    ...
    // ★★★ 드롭바를 만지는 동안에는 안 사라진다 (2026-09-02 · 사용자가 정함)
    //   「드롭바 설정하다가 자막 버튼 또 사라진다.
    //     드롭바 조정할때는 버튼 사라지지 않게 해줘」
    //   자막 크기를 두 번 누르려는데 그 사이에 사라지면 다시 불러와야 한다.
    if (자막판 && !자막판.hidden) return;
    // ★★★ 지금 도는지 안 도는지는 **울릴 때** 본다 (2026-09-02 에 밟음)
    //   「전체화면을 누르면 … x 와 자막버튼이 나오고 안사라진다」
    //   전체화면으로 넘어가는 그 순간엔 유튜브가 상태를 잠깐 흐리게 답한다.
    //   그때 「안 돌고 있네」 하고 시계를 안 걸어 버려서, 영영 안 사라졌다.
    //   ⇒ 시계는 무조건 건다. 울릴 때 그때 도는지 보고 정한다.
    //     멈춰 있으면 그때 안 감추면 된다 — 유튜브도 그렇게 움직인다.
    사라질시계 = setTimeout(() => {
      if (단추칸 && !단추칸.hidden && 돌고있나()) {
        단추칸.classList.add("쉬는중");
  ...
  function 보이기(보일까, 얼마) {
    ...
    if (보일까) 사라질시계걸기(얼마);
      ...
      clearTimeout(사라질시계);
      // ★★★ 사라질 때 드롭바도 접는다 (2026-09-02 · 사용자가 정함)
      //   「드롭바가 열린 상태로 화면을 클릭해서 껐는데, 다시 터치하면 드롭바도 그대로 나온다.
      //     한번 사라지고 다시 나타날때는 드롭바 닫힌 상태로 나오게 해라」
      //   접어 두지 않으면 다시 뜰 때 열린 채로 나와서 화면을 덮는다.
      if (자막판) { 자막판.hidden = true; if (자막메뉴단추) 자막메뉴단추.textContent = "자막 ▾"; }
    }
  }
  ...
  function 톡눌렸다() {
    if (!단추칸 || 단추칸.hidden) return;
    보이기(단추칸.classList.contains("쉬는중"), 톡눌렀을때);
  }
  ...
  function 자막판열기(열까) {
    ...
    // ★ 열려 있는 동안은 시계를 멈추고, 닫으면 다시 건다
    if (열까) clearTimeout(사라질시계);
    else 사라질시계걸기(톡눌렀을때);
  }
