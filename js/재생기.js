...
const 재생기 = (() => {
  ...
  function 입히기() {
    ...
    // ★★★ 조작판이 떠 있는 동안은 **원래 크기**로 돌아간다 (2026-09-02 · 사용자가 정함)
    //   「또 유튜브 재생바 설정 아이콘 이딴거 다 날리지 말고」
    //   채우면 유튜브 재생바·설정이 화면 밖으로 밀려난다. 그래서 —
    //     · 조작판이 없을 때(그냥 볼 때) → 꽉 채운다
    //     · 조작판을 부르면            → 원래 크기로 되돌아가 단추가 다 보인다
    통.classList.toggle("조작판떴다", 보일까);
  ...
  const 손열쇠   = "세진과학.손조절.v2";   /* v1 에 담긴 옛 값은 버린다 */

  // ★★★ 사용자가 폰에서 손으로 맞춘 값이다 (2026-09-02)
  //   「확대 1.12 · 옆 -8 · 위 0」 · 「야 확대 더 올리지 마」 · 「오른쪽은 일부러 남긴거야」
  //   ⇒ 더 채우려 들지 않는다. 이게 정답이다.
  let 손확대 = 1.12;
  let 손배 = 1;      // (안 쓴다 — 가로만 늘리기는 뺐다)
  let 손옆 = -8;
  let 손위 = 0;
  ...
  function 당김맞추기() {
    ...
    // ★★★ 저절로 당기지 않는다 (2026-09-02 · 사용자가 잡음)
    //   「야 화면 비율은 왜 바꿧어」 — 기본값으로 당겨 놓은 것이 잘못이었다.
    //   ⇒ 여기서는 **영상 비율 그대로(16:9)** 화면 안에 얌전히 넣기만 한다.
    //     당기고 싶으면 사람이 조절판으로 직접 당긴다.
    let 틀폭, 틀높이;
    const 가로배 = 1, 세로배 = 1;
    if (폭 / 높이 >= 16 / 9) { 틀높이 = 높이; 틀폭 = 높이 * 16 / 9; }
    else                     { 틀폭 = 폭;   틀높이 = 폭 * 9 / 16; }
