...
const 재생기 = (() => {
  ...
  window.addEventListener("blur", () => {
    // ★★★ 전체화면 단추(자막 ▾ · ✕)가 떠 있을 때만 돈다 (2026-09-22 · 사용자가 잡음)
    //   「지금 컴퓨터에서 (유튜브 스피커) 드래그 안된다」
  ...
  //  ★★★ 유튜브가 안 듣고 있을 때만 우리가 받는다 (2026-09-03 · 사용자가 잡음)
  //    「키보드가 안먹어. 원래는 먹었는데」
  //
  //    유튜브는 **제 틀에 눈길(focus)이 있을 때만** 글쇠를 듣는다.
  //    그런데 영상이 저절로 시작되니 사람이 영상을 누를 일이 없다 —
  //    눈길이 계속 페이지에 있어서 아무 데도 안 갔다.
  //  ⇒ 눈길이 유튜브에 가 있으면 이 손잡이는 **아예 안 불린다**(글쇠가 우리한테 안 온다).
  //    그러니 가로채는 게 아니라 **빈자리를 메우는 것**이다.
