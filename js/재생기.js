...
const 재생기 = (() => {
  ...
  // ★★★ 유튜브 조작판과 발을 맞춘다 (2026-09-02 · 사용자가 정함)
  //   「x 유튜브 재생바 없어질때, 즉 클릭할때 생기고 사라져야 하는데 계속 존재해」
  //
  //   ★ 영상 위의 손짓(누름)은 유튜브 틀이 삼켜서 우리한테 안 온다.
  //     그래서 「눌렀다」 를 직접은 못 안다.
  //   ⇒ 재생 상태로 맞춘다 — 유튜브도 같은 규칙으로 움직인다.
  //     · 멈춰 있으면(일시정지·준비중) 조작판이 떠 있다 → 우리도 보인다
  //     · 돌고 있으면 3초 뒤 조작판이 사라진다 → 우리도 같이 사라진다
  //   ★ 아주 없애지는 않는다. 옅게 남겨서 급하면 누를 수 있게 한다.
  //     없애 버리면 나갈 길이 막혀 갇힌다.
  const 숨는데걸리는시간 = 3200;
  let 숨는시계 = null;
  ...
  function 발맞추기(돌고있나) {
    ...
    clearTimeout(숨는시계);
    if (!돌고있나) { 단추칸.classList.remove("쉬는중"); return; }
    숨는시계 = setTimeout(() => {
      if (단추칸 && !단추칸.hidden) 단추칸.classList.add("쉬는중");
    }, 숨는데걸리는시간);
  }
