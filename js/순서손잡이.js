...
const 순서손잡이 = (() => {
  ...
  function 위메뉴(칸) {
    ...
    칸.querySelectorAll(".메뉴칸[data-아이디]").forEach(단 => 끌기붙이기(단, "가로", async (끈것, 과녁, 앞인가, 형제들) => {
      ...
      // 머리줄의 과목 단추 위에 놓아도 그 과목으로 옮긴다 (세부 과목 탭은 이제 게시판 첫 줄에 있다)
      if (부모 && 과녁.closest(".머리메뉴, .과목띠") && 과녁.dataset.아이디) {
        const 새부모 = 과녁.dataset.아이디;
        const 새과목 = 나무.찾기(새부모);
        if (!새과목 || 새과목.부모 || 새부모 === 부모) return;
        const 차례 = (새과목.마디.아래 || []).map(ㅁ => ㅁ.아이디).concat(끈것.dataset.아이디);
        if (await 저장(새부모, 차례)) { 나무에반영(새부모, 차례); 다시그리기(); 말("「" + 끈것.textContent + "」 을 「" + 새과목.마디.이름 + "」 으로 옮겼다"); }
        return;
      }
      if (과녁.closest("#나무칸")) {
      ...
      if (!과녁.closest(".머리메뉴, .과목띠, .교과띠")) return;
      const 차례 = 줄세우기(형제들, 끈것, 과녁, 앞인가);
