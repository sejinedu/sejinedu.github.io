...
function 지금묶음() {
  ...
  //   전에는 「왼쪽에서 단원을 고르라」 는 빈 화면이었다. 게시판은 들어오자마자 글이 보여야 한다.
  if (!고른아이디) {
    const 과목들 = 나무.목록.filter(갈래맞나);
    const 딸린것 = new Set();
    과목들.forEach(ㅁ => 나무.아래아이디들(ㅁ).forEach(ㄱ => 딸린것.add(ㄱ)));
    return {
...
function 격자그리기() {
  ...
  if (지금무리.length === 0) {
    격자칸.appendChild(안내("아직 올린 강의가 없다", ""));      // ★ 사용자가 정한 말 그대로 (2026-09-22)
    return;
  }
