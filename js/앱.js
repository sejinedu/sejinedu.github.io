...
function 지금층목록() {
  // ★ 맨 위 층에서는 고른 갈래(과학·수학)에 맞는 과목만 보여 준다 (2026-09-02)
  if (경로.length === 0) return 나무.목록.filter(갈래맞나).map(마디 => ({ 마디, 깊이: 0 }));
  const ㅊ = 나무.찾기(경로[0]);
...
function 왼쪽그리기(어디서) {
  ...
  if (마디) {
    ...
    // ★★★ 갈래 단추 — 과학 · 수학 (2026-09-02 · 사용자가 정함)
    //   「과목 선택 아래에 과학, 수학 버튼 만들어라.
    //     크기는 딱 여기 통합과학2 사이즈로 만들어라.
    //     그래서 화면 넘어갈때 이거 두개 큰 제목은 라인좀 맞춰서 이물감 없게 해라」
    //
    //   ★ 크기를 「층이름」 과 똑같이 준다. 그래야 과목 안으로 들어갔다 나올 때
    //     같은 자리에 같은 크기가 놓여 화면이 안 들썩인다.
    //   ★ 아직 수학 과목은 없다. 눌러도 빈 목록이 나오는 게 맞다 —
    //     없는 걸 감추면 왜 안 되는지 알 수가 없다 (지침서 7절 23번).
    const 갈래줄 = document.createElement("div");
    갈래줄.className = "갈래줄";
    갈래들.forEach(ㄱ => {
      const 단 = document.createElement("button");
      단.type = "button";
      단.className = "층이름 갈래" + (지금갈래 === ㄱ ? " 골랐음" : "");
      단.textContent = ㄱ;
      단.addEventListener("click", () => {
        지금갈래 = ㄱ;
        try { localStorage.setItem(갈래열쇠, ㄱ); } catch (오류) {}
        왼쪽그리기();
        격자그리기();
      });
      갈래줄.appendChild(단);
    });
    층머리칸.appendChild(갈래줄);
  }

  // 「과목 추가」 는 맨 위 층에서만 · ★ 그리고 주인일 때만 (2026-09-02)
  손질줄.hidden = 경로.length !== 0 || !파일로고치나;
...
const 파일로고치나 = 주인인가 && !window.저장소가원본;
...
function 과목메뉴그리기() {
  ...
  ["머리과목", "과목띠"].forEach(자리 => {
    ...
    단추("전체", !지금과목 && !고른아이디, 홈으로);
    과목들.forEach(ㅁ => 단추(ㅁ.이름, 지금과목 === ㅁ.아이디, () => 과목고르기(ㅁ.아이디)));
  });
}
