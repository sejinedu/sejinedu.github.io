...
async function 토큰새로받기() {
  ...
  try {
    ...
    //  ★★ 반쪽 파일을 남이 읽지 않게 — 임시 파일에 다 쓰고 한 번에 바꿔 끼운다 (2026-09-23 · 런처 방이 짚음)
    //     바로 쓰면 쓰는 도중에 런처·학생관리가 읽어 반쪽을 보고 「로그인이 깨졌다」 로 안다.
    //     학생관리 세션손.js 세션쓰기() 와 같은 꼴이다.
    //  ★ 적기 직전에 한 번 더 읽는다 — 그 사이 런처가 더 적어 둔 칸(기기 등)을 지우지 않게.
    try {
      const 지금것 = 세션읽기();
      const 새것 = Object.assign({}, ㅅ, 지금것, {
        토큰: 로그인.토큰,
        새로고침표: 로그인.새로고침표 || 지금것.새로고침표 || ㅅ.새로고침표,
        만료: ㄱ.expires_at || 지금것.만료 || ㅅ.만료,
      });
      const 임시 = 세션파일 + ".홈페이지가쓰는중";
      fs.writeFileSync(임시, JSON.stringify(새것, null, 1), "utf8");
      fs.renameSync(임시, 세션파일);
    } catch (오류) { /* 못 적어도 이번 판은 돈다 — 다음에 새로 받을 때 다시 적는다 */ }
    return { 됐나: true };
  } catch (오류) {
...
const 서버 = http.createServer((req, res) => {
  ...
  // ---- 런처 로그인을 이 컴퓨터 화면에 물려준다 (2026-09-22) ----
  //   사용자: 「로그인은 세도비 런처 로그인 아이디 비번이랑 같은 거라고」
  //   ★ 8777 화면이 따로 로그인하게 하면 안 된다. 서버가 들고 있는 런처 로그인을 준다.
  //   ★ 출입증(짧은 토큰)만 준다. 새로고침표는 안 준다 — 화면이 그걸로 새로 받으면
  //     런처가 들고 있는 표가 무효가 돼서 런처가 로그아웃된다. 새로 받는 건 늘 서버가 한다.
  //   ★ 이 컴퓨터 안에서 온 부름만 받는다 (주소가 127.0.0.1/localhost 인 화면).
  if (길 === "/로그인/표") {
    const 손님 = String(req.headers.host || "");
    const 어디서 = String(req.headers.origin || "");
    const 안이다 = ㄱ => /^(127\.0\.0\.1|localhost):\d+$/.test(ㄱ);
    if (!안이다(손님) || (어디서 && !안이다(어디서.replace(/^https?:\/\//, "")))) { res.writeHead(403); return res.end(); }
    return void 로그인챙기기().then(됐나 => {
      const 살핀것 = 토큰살펴보기(로그인.토큰);
      if (!됐나 || !살핀것.산다) return 답하기(res, { 됐나: false });
      답하기(res, { 됐나: true, 토큰: 로그인.토큰, 메일: 로그인.메일, 누구: 로그인.누구, 남은초: (살핀것.남은분 || 30) * 60 });
    }).catch(() => 답하기(res, { 됐나: false }));
  }
