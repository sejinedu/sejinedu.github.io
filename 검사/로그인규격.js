...
봄(/function 선생관리되나/.test(창) && /원장인가/.test(창) && /권한\.초대/.test(창), "선생 관리 — 원장이거나 「초대」 권한");
...
// 6) 가입 끝 「어떻게 쓰나요?」 · 홈피 등급과 앱 자격은 따로 (2026-09-24 스스로 원장)
const 가입줄 = 규격.split("\n").find(ㄱ => /^- 회원 가입:/.test(ㄱ)) || "";
const 쓰임단추 = [...규격.slice(규격.indexOf(가입줄)).split("\n").slice(1, 4).join("\n").matchAll(/단추 `([^`]+)`/g)].map(ㅁ => ㅁ[1]);
봄(가입줄.includes("어떻게 쓰나요?") && 창.includes('"어떻게 쓰나요?"'), "가입 끝에 「어떻게 쓰나요?」");
봄(쓰임단추.length === 2 && 쓰임단추.every(ㄱ => 창.includes('"' + ㄱ + '"')), "쓰임 단추 이름이 규격과 같다 — " + 쓰임단추.join(" / "));
봄(/학원_만들기", \{ p_이름: 이름 \}/.test(창), "원장 → rpc/학원_만들기(p_이름)");
봄(/function 쓰임으로\(\) \{[\s\S]{0,120}?ㅅ\.학원 && ㅅ\.학원\.학원/.test(창), "초대받아 이미 학원에 들어왔으면 쓰임을 건너뛴다");
봄(/return !\(ㅅ\.학원 && ㅅ\.학원\.학원\);/.test(창) && !/function 선생관리되나[\s\S]{0,300}?등급 === "teacher"/.test(창), "선생 관리 — 학원 없는 사람은 누구나 (등급 조건 없음)");
봄(!/site_me/.test((/async function 학원읽기\(\) \{[\s\S]*?\n  \}/.exec(회원) || [""])[0]), "학원_나 뒤에 홈피 등급을 다시 읽지 않는다 — 등급은 관리자만");
const 회원관리 = fs.readFileSync(path.join(집, "js", "회원관리.js"), "utf8");
봄(/학원관리_목록/.test(회원관리) && /학원관리_정지", \{ p_학원: ㅎ\.학원, p_정지: 정지 \}/.test(회원관리), "회원 관리 안에 학원 목록 · 정지");
