...
const 초대단추들 = [...new Set([...초대절.split("권한은 **각 앱에서**")[0].replace(/```[\s\S]*?```/g, "").matchAll(/`([^`]+)`/g)].map(ㅁ => ㅁ[1]).filter(ㄱ => !/[(…_]/.test(ㄱ) && !/^https?:/.test(ㄱ) && !/\.sql$/.test(ㄱ)))];
...
const 받는곳 = (/https:\/\/sejinedu\.github\.io\/download\.html/.exec(초대절) || [])[0];
...
// ② 가입 안내 글 — 규격 「② 가입 안내」 의 글 덩어리(``` 안)와 **줄 그대로** 견준다 (2026-09-25 한 방향)
const 규격안내 = (() => {
  const ㅈ = 초대절.indexOf("**가입 안내**"); if (ㅈ < 0) return null;
  const ㅁ = /```\n([\s\S]*?)\n\s*```/.exec(초대절.slice(ㅈ)); if (!ㅁ) return null;
  const 줄들 = ㅁ[1].split("\n"); const 들여 = Math.min(...줄들.filter(ㄱ => ㄱ.trim()).map(ㄱ => ㄱ.match(/^ */)[0].length));
  return 줄들.map(ㄱ => ㄱ.slice(들여).replace(/\s+$/, ""));
})();
const 홈피안내 = (() => {
  const ㅁ = /const 가입안내글 = \(\) => (\[[\s\S]*?\]\.join\("\\n"\));/.exec(초대); if (!ㅁ) return null;
  const 틀2 = { 채널이름: "채널 이름", 런처받는곳: 받는곳 }; vm.createContext(틀2);
  return vm.runInContext(ㅁ[1], 틀2).split("\n");
})();
...
봄(규격.includes("rpc/학원관리_한도(p_학원, p_한도MB)") && /"학원관리_한도", \{ p_학원: ㅎ\.학원, p_한도MB: MB \}/.test(회원관리) && /한도MB/.test(회원관리) && /Math\.round\(GB \* 1024\)/.test(회원관리), "채널 탭 — 사람마다 저장 한도(GB 로 받아 MB 로) · 기본으로 = null");
